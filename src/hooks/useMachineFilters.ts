...
export const useMachineFilters = (machines: Machine[]) => {
  ...
  const resetAllFilters = () => {
    setSearchValue(''); setCategoryValue('ALL'); setSortValue('recent');
  ...
  const filteredMachines = useMemo(() => {
    ...
    return machines.filter(m => {
      ...
      if (minCapacityValue || maxCapacityValue) {
          ...
          if (valCap === 0 && m.categoria_tarea === 'Bombas') {
              const matchM = m.titulo.match(/(\d{2})(?:z|m|x|\-|\s)/i);
              if (matchM) valCap = parseFloat(matchM[1]);
          }
          if (valCap === 0 && m.categoria_tarea === 'Camiones Trompo') {
      ...
      if (m.categoria_tarea === 'Bombas') {
          machineHours = m.uso_bomba || 0;
          machineMiles = m.uso_motor || 0;
      } else if (['Camiones Volteo', 'Camiones Trompo', 'Camiones Pipa', 'Tractocamiones', 'Gruas Titanes'].includes(m.categoria_tarea)) {
      ...
      if (m.categoria_tarea === 'Gruas Articuladas' || m.categoria_tarea === 'Gruas Titanes') {
      ...
      if (m.categoria_tarea === 'Retroexcavadoras') {
