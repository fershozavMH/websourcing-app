...
export default function MachineCard({ machine }: { machine: Machine }) {
  ...
  // Validación: ¿Ya fue enviada al MVP?
  const yaEnviadoMvp = !!(machine as any).enviado_mvp;

  const enviarAMVP = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setEnviandoMvp(true);
    ...
    try {
      ...
      if (data.success) {
        alert('¡Máquina enviada con éxito al MVP!');
        (machine as any).enviado_mvp = true;
      } else {
        alert(`Error al enviar: ${data.error}`);
      }
    } catch (error) {
